...
Class RestComponent extends Object {
	...
	public function response ($data = array()) {
		...
		foreach($_GET as $_gkey=>$_gval){
			switch($_gkey){
				case 'url': case 'limit': case 'page': break; //Whitelist these keys
				default:
					if(preg_match('/%/',$_gval)){
						$filter[$this->Model->name.'.'.$_gkey.' LIKE'] = $_gval;
					}else{
						$filter[$this->Model->name.'.'.$_gkey] = $_gval;
					}
					array_push($_filter,$_gkey.'='.$_gval);
				break;			
			}
		}
		$count =$this->Model->find('count',array('conditions'=>$filter));
		...
		$response = array(
			'meta' => array(
				...
				'prev'=> (!$prev?null:($protocol.$server['http_host'].$base.'?'.implode(array_merge($_filter,(array)('page='.($prev))),'&'))),
				'next'=> (!$next?null:($protocol.$server['http_host'].$base.'?'.implode(array_merge($_filter,(array)('page='.($next))),'&'))),
				'last'=> (!$last?null:($protocol.$server['http_host'].$base.'?'.implode(array_merge($_filter,(array)('page='.($last))),'&'))),
				'page'=> $page,
				'pages'=> $last,
				'count'=> $count,
